...
class MCPProtocolConnection:
    ...
    async def send_packet(self, packet: MCPacket) -> bool:
        ...
        async with self._write_lock:
            try:
                ...
                # 压缩 (如果启用且数据足够大)
                if self._compression == CompressionState.ENABLED and len(data) >= self._compression_threshold:
                    compressed = zlib.compress(data)
                    data = VarInt.encode(len(compressed)) + compressed
        ...
        while self._running:
            try:
                ...
                if packet:
                    ...
                    # 处理心跳
                    if packet.packet_id == PacketID.KEEP_ALIVE_PACKET:
    ...
    async def _handle_keepalive(self, packet: MCPacket) -> None:
        """处理心跳包"""
        # 立即回复相同的心跳 ID
        keepalive_id = struct.unpack('>q', packet.data.get('keep_alive_id', b'\x00' * 8))[0]
