...
class MCPProtocolCodec:
    ...
    数据包格式 (RakNet):
    [Message Code: 2 bytes] [Flags: 1 byte] [Session ID: 4 bytes] [Data: variable]
    ...
    HEADER_SIZE = 7  # 2 (code) + 1 (flags) + 4 (session)
    ...
    def encode(self, packet: MCPPacket) -> bytes:
        ...
        try:
            ...
            # 加密 (CH 消息)
            if (packet.direction == PacketDirection.CLIENT_TO_SERVER and 
                self.xxtea_key):
                data = self._xxtea_encrypt(data)
                flags |= PacketFlag.ENCRYPTED
            
            # 构建包头
            header = struct.pack(
                '<HBI',  # 小端: 2字节无符号短整, 1字节, 4字节无符号整
                packet.msg_code,
                flags,
                packet.session_id
            )
