...
class MCPRakNetGateway:
    ...
    def __init__(self, config: Optional[GatewayConfig] = None):
        ...
        # 服务器
        self.server: Optional[aiorak.Server] = None
        self._running = False
    ...
    async def _on_connect(self, conn: Connection) -> None:
        """客户端连接回调"""
    ...
    async def _on_data(self, conn: Connection, data: bytes) -> None:
        """数据接收回调"""
        try:
            conn_id = f"{conn.address[0]}:{conn.address[1]}"
    ...
    async def _handle_bridge_mode(self, conn: Connection, packet: MCPPacket) -> None:
        """桥接模式处理"""
        # TODO: 实现桥接到 MC 的逻辑
    ...
    async def _handle_proxy_mode(self, conn: Connection, packet: MCPPacket) -> None:
        """代理模式处理"""
        # TODO: 透传到真实 MNW 服务器
        logger.debug(f"Proxy mode: {packet.msg_code}")
    
    async def _handle_standalone_mode(self, conn: Connection, packet: MCPPacket) -> None:
        """独立模式处理"""
        # TODO: 处理本地房间逻辑
