...
from ..mcp_mc.client import MCPMinecraftClient, MCClientConfig, PlayerPosition
...
class MCPBridge:
    ...
    def _register_handlers(self) -> None:
        ...
        async def on_mc_join():
            ...
            await self._trigger_event('mc_joined')
        ...
        @self.mc_client.on('chat')
        async def on_mc_chat(message, position):
            self.stats.chat_messages += 1
            await self._forward_chat_mc_to_mnw(message)
        
        @self.mc_client.on('position')
        async def on_mc_position(pos):
            await self._sync_mc_position(pos)
        ...
        async def on_mnw_enter():
            ...
            await self._trigger_event('mnw_entered')
        ...
        @self.mnw_client.on('chat')
        async def on_mnw_chat(data):
            self.stats.chat_messages += 1
            await self._forward_chat_mnw_to_mc(data)
        
        @self.mnw_client.on('move')
        async def on_mnw_move(data):
            await self._sync_mnw_position(data)
    ...
    async def _sync_mc_position(self, pos: PlayerPosition) -> None:
        """同步 MC 位置到 MNW"""
        if not self.mnw_client or not self.mnw_client.is_in_game:
            return
        ...
        if self.config.log_sync:
            logger.debug(f"MC->MNW Position sync: ({pos.x:.2f}, {pos.y:.2f}, {pos.z:.2f})")
    
    async def _sync_mnw_position(self, data: Dict[str, Any]) -> None:
        """同步 MNW 位置到 MC"""
        if not self.mc_client or not self.mc_client.is_in_game:
            return
        ...
        await self.mc_client.update_position(
            x=x, y=y, z=z,
        ...
        if self.config.log_sync:
            logger.debug(f"MNW->MC Position sync: ({x:.2f}, {y:.2f}, {z:.2f})")
    
    async def _forward_chat_mc_to_mnw(self, message: str) -> None:
        """转发 MC 聊天到 MNW"""
        if not self.mnw_client or not self.mnw_client.is_in_game:
            return
        ...
        try:
            ...
            await self.mnw_client.send_chat(f"[MC] {text}")
            
            if self.config.log_sync:
                logger.debug(f"MC->MNW Chat: {text[:50]}")
                
        except Exception as e:
            logger.error(f"Failed to forward MC chat: {e}")
    
    async def _forward_chat_mnw_to_mc(self, data: Dict[str, Any]) -> None:
        """转发 MNW 聊天到 MC"""
        if not self.mc_client or not self.mc_client.is_in_game:
            return
        ...
        try:
            ...
            await self.mc_client.send_chat(f"[MNW] {sender}: {message}")
            
            if self.config.log_sync:
                logger.debug(f"MNW->MC Chat: {sender}: {message[:50]}")
                
        except Exception as e:
            logger.error(f"Failed to forward MNW chat: {e}")
