...
@dataclass
class LoginSuccessPacket(MCPacket):
    uuid: str = ""
    username: str = ""

    def __post_init__(self):
        self.packet_id = PacketID.LOGIN_SUCCESS

    @classmethod
    def decode(cls, data: bytes) -> 'LoginSuccessPacket':
        stream = io.BytesIO(data)
        packet_id = VarInt.decode_stream(stream)
        uuid = MCUUID.decode_stream(stream)
        username = MCString.decode_stream(stream)
        return cls(packet_id=packet_id, uuid=uuid, username=username, data={
            'uuid': uuid,
            'username': username,
        })
...
@dataclass
class SetCompressionPacket(MCPacket):
    threshold: int = -1

    def __post_init__(self):
        self.packet_id = PacketID.SET_COMPRESSION

    @classmethod
    def decode(cls, data: bytes) -> 'SetCompressionPacket':
        stream = io.BytesIO(data)
        packet_id = VarInt.decode_stream(stream)
        threshold = VarInt.decode_stream(stream)
        return cls(packet_id=packet_id, threshold=threshold, data={'threshold': threshold})
...
class JoinGamePacket(MCPacket):
    ...
    @classmethod
    def decode(cls, data: bytes) -> 'JoinGamePacket':
        stream = io.BytesIO(data)
        packet_id = VarInt.decode_stream(stream)
        entity_id = MCInt.decode_stream(stream) if stream.tell() < len(data) else 0
        is_hardcore = MCBoolean.decode_stream(stream) if stream.tell() < len(data) else False
        gamemode = MCUnsignedByte.decode_stream(stream) if stream.tell() < len(data) else 0
        previous_gamemode = MCByte.decode_stream(stream) if stream.tell() < len(data) else 0
        return cls(
            packet_id=packet_id,
            entity_id=entity_id,
            is_hardcore=is_hardcore,
            gamemode=gamemode,
            previous_gamemode=previous_gamemode,
            data={
                'entity_id': entity_id,
                'gamemode': gamemode,
            }
...
class PlayerPositionAndLookPacket(MCPacket):
    ...
    @classmethod
    def decode(cls, data: bytes) -> 'PlayerPositionAndLookPacket':
        stream = io.BytesIO(data)
        packet_id = VarInt.decode_stream(stream)
        x = MCDouble.decode_stream(stream)
        y = MCDouble.decode_stream(stream)
        z = MCDouble.decode_stream(stream)
        yaw = MCFloat.decode_stream(stream)
        pitch = MCFloat.decode_stream(stream)
        flags = MCUnsignedByte.decode_stream(stream)
        teleport_id = VarInt.decode_stream(stream)
        return cls(packet_id=packet_id, x=x, y=y, z=z, yaw=yaw, pitch=pitch, flags=flags, teleport_id=teleport_id, data={
            'x': x,
            'y': y,
            'z': z,
            'yaw': yaw,
...
class KeepAlivePacket(MCPacket):
    ...
    @classmethod
    def decode(cls, data: bytes) -> 'KeepAlivePacket':
        stream = io.BytesIO(data)
        packet_id = VarInt.decode_stream(stream)
        keep_alive_id = MCLong.decode_stream(stream)
        return cls(packet_id=packet_id, keep_alive_id=keep_alive_id, data={'keep_alive_id': keep_alive_id})
...
class ChatMessagePacket(MCPacket):
    ...
    @classmethod
    def decode(cls, data: bytes) -> 'ChatMessagePacket':
        stream = io.BytesIO(data)
        packet_id = VarInt.decode_stream(stream)
        json_data = MCString.decode_stream(stream) if stream.tell() < len(data) else ""
        position = MCUnsignedByte.decode_stream(stream) if stream.tell() < len(data) else 0
        sender = MCUUID.decode_stream(stream) if len(data) - stream.tell() >= 16 else ""
        return cls(packet_id=packet_id, json_data=json_data, position=position, sender=sender, data={
            'json_data': json_data,
            'position': position,
            'sender': sender,
        })
...
@dataclass
class ClientChatMessagePacket(MCPacket):
    """客户端聊天消息数据包 (C->S)"""
    message: str = ""
    
    def __post_init__(self):
        self.packet_id = PacketID.CHAT_MESSAGE
    
    def encode(self) -> bytes:
        result = VarInt.encode(self.packet_id)
        result += MCString.encode(self.message)
        return result


@dataclass
class PlayerPositionPacket(MCPacket):
    """玩家位置数据包 (C->S)"""
    x: float = 0.0
    y: float = 0.0
    z: float = 0.0
    ...
    def __post_init__(self):
        self.packet_id = PacketID.PLAYER_POSITION
    
    def encode(self) -> bytes:
        result = VarInt.encode(self.packet_id)
        result += MCDouble.encode(self.x)
        result += MCDouble.encode(self.y)
        result += MCDouble.encode(self.z)
        result += MCBoolean.encode(self.on_ground)
        return result


@dataclass
class PlayerPositionAndRotationPacket(MCPacket):
    """玩家位置和旋转数据包 (C->S)"""
    x: float = 0.0
    y: float = 0.0
    z: float = 0.0
    ...
    def __post_init__(self):
        self.packet_id = PacketID.PLAYER_POSITION_AND_ROTATION
    ...
    def encode(self) -> bytes:
        ...
        result += MCDouble.encode(self.y)
        result += MCDouble.encode(self.z)
        result += MCFloat.encode(self.yaw)
        result += MCFloat.encode(self.pitch)
        result += MCBoolean.encode(self.on_ground)
        return result


@dataclass
class PlayerRotationPacket(MCPacket):
    """玩家旋转数据包 (C->S)"""
    yaw: float = 0.0
    pitch: float = 0.0
    on_ground: bool = True
    ...
    def __post_init__(self):
        self.packet_id = PacketID.PLAYER_ROTATION
...
PACKET_REGISTRY: Dict[int, type] = {
    ...
    PacketID.LOGIN_SUCCESS: LoginSuccessPacket,
    PacketID.SET_COMPRESSION: SetCompressionPacket,
    ...
    PacketID.KEEP_ALIVE_PACKET: KeepAlivePacket,
    PacketID.CHAT_MESSAGE_PACKET: ChatMessagePacket,
    PacketID.SYSTEM_CHAT_MESSAGE: ChatMessagePacket,
    PacketID.PLAYER_CHAT_MESSAGE: ChatMessagePacket,
    PacketID.CHAT_MESSAGE: ClientChatMessagePacket,
    PacketID.PLAYER_POSITION: PlayerPositionPacket,
    PacketID.PLAYER_POSITION_AND_ROTATION: PlayerPositionAndRotationPacket,
    PacketID.PLAYER_ROTATION: PlayerRotationPacket,
}
